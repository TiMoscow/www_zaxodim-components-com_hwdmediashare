...
class hwdMediaShareVideos extends JObject
{
	...
	public function processMp4($process, $fileType, $size)
	{
                ...
                try
                {
                        ...
                        switch ($config->get('process_vpreset')) //prohody
                        {
                                case '1':
                                case '2':
                                case '3':
                        ...
                        //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                ...
                                //$input1 = $config->get('path_ffmpeg', '/usr/bin/ffmpeg')." -y -i $pathSource -s $wxh $pad  -vcodec libvpx -g 120 -rc_lookahead 16 $opt_quality $opt_speed -profile 0 -qmax 51 -qmin 11 $opt_slices -vb 2M  -b ".$bitrate."k -pass 1 -an -f webm $pathDest 2>&1";
                                $input1 = $config->get('path_ffmpeg', '/usr/bin/ffmpeg')." -y -i  $pathSource -strict experimental -acodec aac -ac 2 -ab 192k -s $wxh $aspect_option -r 24000/1001 -vcodec libx264 -pass 1 -b:v ".$vbit."k -minrate ".$min."k -maxrate ".$max."k -bufsize ".$buff." -crf $crf  -preset fast  -f mp4  -threads 0  $pathDest 2>&1";
                                exec($input1, $output1);
                                // $input2 = $config->get('path_ffmpeg', '/usr/bin/ffmpeg')." -y -i $pathSource -s $wxh $pad  -vcodec libvpx -g 120 -rc_lookahead 16 $opt_quality $opt_speed -profile 0 -qmax 51 -qmin 11 $opt_slices -vb 2M -maxrate 24M -minrate 100k $opt_arnr  -b ".$bitrate."k -pass 2 -acodec libvorbis -ab 90k -f webm $pathDest 2>&1";
                                $input2 = $config->get('path_ffmpeg', '/usr/bin/ffmpeg')." -y -i  $pathSource -strict experimental -acodec aac -ac 2 -ab 192k -s $wxh $aspect_option -r 24000/1001 -vcodec libx264 -pass 2 -b:v ".$vbit."k -minrate ".$min."k -maxrate ".$max."k -bufsize ".$buff." -crf $crf  -preset fast  -f mp4  -threads 0  $pathDest 2>&1";
                                exec($input2, $output2);
